...
class MaterialCreateContract(
    private val dataLoader: DataLoader
) : ScreenContract {
    override val screenKey = "material-create"
    ...
    private class SubmitFormHandler(
        ...
        override val requiredPermission: String? = "materials:create"
        ...
        override suspend fun execute(context: EventContext): EventResult {
            ...
            return when (val result = dataLoader.submitData("/api/v1/materials", body, "POST")) {
                is Result.Success -> {
                    val materialId = result.data?.get("id")?.jsonPrimitive?.content ?: ""
                    EventResult.NavigateTo("material-detail", mapOf("id" to materialId))
                }
